...
import type { StoryItem, MatchSource, QAStatus } from '../types';
import { CheckCircle, AlertTriangle, XCircle, Activity } from 'lucide-react';

const QA_CLASSES: Record<QAStatus['status'], string> = {
...
const QA_ICONS: Record<QAStatus['status'], React.ReactNode> = {
  pass: <CheckCircle size={16} />,
  warn: <AlertTriangle size={16} />,
  fail: <XCircle size={16} />,
};

function QABadge({ status, score }: { status: QAStatus['status']; score: number }) {
  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium shadow-sm whitespace-nowrap ${QA_CLASSES[status]}`}>
      {QA_ICONS[status]}
...
function ScoreDisplay({ source }: { source: MatchSource }) {
  return (
    <div className="flex items-center justify-center bg-slate-900/50 p-2 rounded-xl border border-slate-700/50 shadow-inner">
      {source.homeBadge
        ? <img src={source.homeBadge} alt={source.homeTeam} className="w-10 h-10 object-contain drop-shadow-md" />
        : <div className="w-10 h-10 border border-slate-700 rounded-lg flex justify-center items-center text-xs opacity-50">{source.homeTeam.substring(0, 3)}</div>}
      ...
      {source.awayBadge
        ? <img src={source.awayBadge} alt={source.awayTeam} className="w-10 h-10 object-contain drop-shadow-md" />
        : <div className="w-10 h-10 border border-slate-700 rounded-lg flex justify-center items-center text-xs opacity-50">{source.awayTeam.substring(0, 3)}</div>}
    </div>
  );
}
...
export const StoryCard: React.FC<StoryCardProps> = React.memo(({ item, onClick, isActive }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        ...
        <QABadge status={qa.status} score={qa.score} />
      </div>
